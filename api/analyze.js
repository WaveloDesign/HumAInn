export default async function handler(req, res) {
  ...
  const prompts = [
    ...
    `Συγκρίνοντας με ανθρώπινο γραπτό, πόσο AI είναι αυτό; Δώσε ΜΟΝΟ JSON χωρίς backticks:\n{"score":<0-100>,"signals":["<παρ1>","<παρ2>","<παρ3>"],"reason":"<1 πρόταση>"}\n\nΚείμενο:\n${sample}`,
  ];

  async function geminiCall(prompt) {
    const r = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/models/gemini-pro:generateContent?key=${geminiKey}`,
      {
        method: 'POST',
  ...
  try {
    ...
    return res.status(200).json({ ai_score: avgScore, verdict, reason, top_signals: allSignals, ensemble: results.map(j => j.score) });
  } catch (e) {
